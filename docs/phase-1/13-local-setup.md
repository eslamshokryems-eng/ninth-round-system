...
This project targets **Expo SDK 57** specifically because the Expo Go app only supports the current SDK version at any given time — an older SDK (this project originally targeted SDK 51) silently fails to load in today's Expo Go. If you hit an SDK-mismatch error despite following this guide, check `apps/mobile/package.json`'s `expo` version against whatever Expo Go currently supports (visible in the Expo Go app's own settings screen) and let me know — the ecosystem moves fast enough that this may need bumping again by the time you read this.
