...
create policy "admins manage categories" on exercise_categories for all
...
create policy "trainers insert exercises" on exercises for insert
  with check ((auth.jwt() ->> 'role') in ('trainer', 'admin'));
...
create policy "anyone reads published programs" on programs for select
  using (is_published = true or created_by = auth.uid() or (auth.jwt() ->> 'role') = 'admin');
  ...
  using (
    exists (
      ...
      where p.id = program_weeks.program_id
        and (p.is_published or p.created_by = auth.uid() or (auth.jwt() ->> 'role') = 'admin')
    )
  );
...
create policy "trainers read assigned clients' user_programs" on user_programs for select
  using (
    exists (
      select 1 from trainer_clients tc
      where tc.client_id = user_programs.profile_id
        and tc.trainer_id = auth.uid()
        and tc.status = 'active'
    )
  );
create policy "admins manage user_programs" on user_programs for all
  using ((auth.jwt() ->> 'role') = 'admin');
