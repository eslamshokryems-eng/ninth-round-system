...
import { BackButton, Card, IconButton, Text } from "@9thround/ui/native";
...
const PAYMENT_METHOD_LABEL_KEY: Record<Receipt["paymentMethod"], string> = {
...
/** Receipts (sidebar item) — a read-only, chronological view of every membership_payments row for the branch. */
      ...
      <ScrollView className="flex-1" contentContainerClassName="gap-6 px-6 py-6">
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center gap-3">
            ...
            <Text variant="display">{t("reception.receipts.title")}</Text>
          </View>
          <IconButton
            name="menu"
        ...
        {isLoading ? (
          ...
          <Text variant="body" color="muted" style={{ textAlign: "center" }}>
            {t("reception.receipts.empty")}
          </Text>
        ) : (
          <View className="gap-3">
            {receipts.map((receipt) => (
              <Card key={receipt.paymentId} className="gap-1">
                <View className="flex-row items-center justify-between">
                  <Text variant="title">{receipt.memberFullName}</Text>
                  <Text variant="title" color="gold">
                    {receipt.amount.toLocaleString()} EGP
                  </Text>
                </View>
                <Text variant="caption" color="muted">
                  {t("reception.receipts.receiptNumber")} {receipt.receiptNumber} · {receipt.membershipNumber}
                </Text>
                <Text variant="caption" color="muted">
                  {receipt.paymentDate} · {t(PAYMENT_METHOD_LABEL_KEY[receipt.paymentMethod])}
                </Text>
              </Card>
            ))}
          </View>
