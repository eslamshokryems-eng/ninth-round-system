...
export function ProgramCard({ program, lang }: { program: Program; lang: Lang }) {
  const t = dict(lang);

  return (
    <Link
      href={href(lang, `/programs/${program.slug}`)}
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-6">
        <h3 className="font-display text-xl uppercase tracking-wide text-bone">{program.name[lang]}</h3>
        <p className="mt-2 flex-1 text-sm leading-relaxed text-ash">{program.short[lang]}</p>
        <p className="mt-4 text-xs uppercase tracking-wider text-ash/70">{t.programCard.who}</p>
        <p className="mt-1 text-sm text-ash">{program.who[lang]}</p>
        <span className="mt-5 inline-flex items-center gap-1.5 font-display text-sm font-semibold uppercase tracking-wide text-blood-bright">
          ...
          <svg
            ...
            fill="none"
            aria-hidden="true"
            className="u-flip transition-transform group-hover:translate-x-1"
          >
            <path
