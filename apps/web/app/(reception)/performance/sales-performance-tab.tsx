...
/**
 * Revenue attributed via memberships.sold_by — who closed each sale.
 * Ranked by collected_revenue (the primary KPI: what was actually paid),
 * server-aggregated by get_sales_performance() so double-payment
 * memberships are never double-counted.
 */
export function SalesPerformanceTab() {
  ...
  const load = useCallback(async () => {
    ...
    const [rowsResult, targetsResult] = await Promise.all([
      reception.getSalesPerformance.execute({
        branchId,
        ...range,
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="flex flex-wrap items-end gap-4">
          ...
          <div className="w-56">
            <StaffPicker selected={staffFilter} onSelect={setStaffFilter} label="Employee" />
          </div>
          <div className="w-40">
            <SelectField
              label="Program"
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            ...
            <StatCard label="Employees Above Target" value={`${aboveTargetCount} / ${rows.length}`} />
          </div>
          ...
          <Card>
            <h2 className="mb-4 text-sm font-semibold text-ink">Sales Performance ({rows.length})</h2>
            {rows.length === 0 ? (
              <p className="text-sm text-muted">No attributed sales in this range.</p>
            ) : (
